...
def obtenerDatosUsuario(request):
    idUsuario = request.GET.get('idUsu')
    celuUsuario = request.GET.get('celu')
    profesionUsuario = request.GET.get('profe')
    perfilUsuario = request.GET.get('perf')
    ...
    try:
        ...
        return JsonResponse({ 
            'userName' : objUsu.username,                           
            'nombre':objUsu.first_name,
            'apellido':objUsu.last_name,
            'email':objUsu.email,            
            'profesion': profesionUsuario,
            'celular': celuUsuario,
...
def actualizarUsuario(request):
    ...
    if request.method == 'POST':
        ...
        datosUsuario.objects.filter(
            usrRel = User.objects.get(id=idUsuario)
        ).update(
            profesion = profesionUsuario,
            nroCelular = nroCelular,
            perfilUsuario = perfilUsuario
